...
public class MultipleDaysWeather extends AbstractItem<MultipleDaysWeather, MultipleDaysWeather.MyViewHolder> {
  ...
  protected static class MyViewHolder extends FastAdapter.ViewHolder<MultipleDaysWeather> {
    ...
    public void bindView(@NonNull MultipleDaysWeather item, @NonNull List<Object> payloads) {
      ...
      if (AppUtil.isChinese(context)) {           //AppUtil.isRTL(context)
        binding.dayNameTextView.setText(Constants.DAYS_OF_WEEK_CHINESE[calendar.get(Calendar.DAY_OF_WEEK) - 1]);
        binding.dateTextView.setText(String.format(Locale.CHINESE, "%s %d",
                Constants.MONTH_NAME_CHINESE[calendar.get(Calendar.MONTH)], calendar.get(Calendar.DAY_OF_MONTH)));
      } else {
        binding.dayNameTextView.setText(Constants.DAYS_OF_WEEK[calendar.get(Calendar.DAY_OF_WEEK) - 1]);
        binding.dateTextView.setText(String.format(Locale.ENGLISH, "%s %d",
