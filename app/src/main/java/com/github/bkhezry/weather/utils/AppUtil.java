...
public class AppUtil {
   ...
   * @param isChinese       boolean value
   * @return String weather status
   */
  public static String getWeatherStatus(int weatherCode, boolean isChinese) {
    if (weatherCode / 100 == 2) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[0];
      } else {
        return Constants.WEATHER_STATUS[0];
      }
    } else if (weatherCode / 100 == 3) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[1];
      } else {
        return Constants.WEATHER_STATUS[1];
      }
    } else if (weatherCode / 100 == 5) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[2];
      } else {
        return Constants.WEATHER_STATUS[2];
      }
    } else if (weatherCode / 100 == 6) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[3];
      } else {
        return Constants.WEATHER_STATUS[3];
      }
    } else if (weatherCode / 100 == 7) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[4];
      } else {
        return Constants.WEATHER_STATUS[4];
      }
    } else if (weatherCode == 800) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[5];
      } else {
        return Constants.WEATHER_STATUS[5];
      }
    } else if (weatherCode == 801) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[6];
      } else {
        return Constants.WEATHER_STATUS[6];
      }
    } else if (weatherCode == 803) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[7];
      } else {
        return Constants.WEATHER_STATUS[7];
      }
    } else if (weatherCode / 100 == 8) {
      if (isChinese) {
        return Constants.WEATHER_STATUS_CHINESE[8];
      } else {
  ...
  public static boolean isChinese(Context context){
    ...
    if(locale.getLanguage().equals("zh"))return true;
    else return false;
  }
